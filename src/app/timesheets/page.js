...
export default function TimesheetsPage() {
  ...
  const getWorker = (id) => workers.find((w) => w.id === id) || { id: '0', firstName: 'Unknown', lastName: '', hourlyRate: 0 };
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <th>Notes</th>
                    <th>{t('timesheetsAdditions.source')}</th>
                  </tr>
                </thead>
                <tbody>
                  {logsWithCalculations.map((log) => (
                    <tr key={log.id}>
                      ...
                      <td>
                        {log.autoGenerated ? (
                          <Link href="/tasks" className="badge badge-accent" style={{ fontSize: '10px', padding: '1px 6px', textDecoration: 'none' }}>
                            🔗 {t('timesheetsAdditions.fromTask')}
                          </Link>
                        ) : (
                          <span className="badge badge-neutral" style={{ fontSize: '10px', padding: '1px 6px' }}>
                            {t('timesheetsAdditions.manual')}
                          </span>
                        )}
                      </td>
                    </tr>
            ...
            {logsWithCalculations.map((log) => (
              <div key={log.id} className="mobile-card-item">
                ...
                <div className="mobile-card-row" style={{ marginTop: 8 }}>
                  <span className="mobile-card-label">{t('timesheetsAdditions.source')}</span>
                  {log.autoGenerated ? (
                    <Link href="/tasks" className="badge badge-accent" style={{ fontSize: '10px', padding: '1px 6px', textDecoration: 'none' }}>
                      🔗 {t('timesheetsAdditions.fromTask')}
                    </Link>
                ...
                >
                  {workers.map((w, index) => (
                    <option key={w.id} value={index}>
                      {w.firstName} {w.lastName} ({w.hourlyRate} RON/h)
                    </option>
                  ))}
                </select>
              </div>
